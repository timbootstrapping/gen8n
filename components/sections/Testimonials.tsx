...
export default function Testimonials() {
  return (
    <section className="py-10 px-4 sm:px-8 lg:px-20 space-y-12">
      <h2 className="text-center text-3xl font-semibold">Loved by builders</h2>
      <div className="grid md:grid-cols-3 gap-6">
        {testimonials.map(({ name, avatar, text }, idx) => (
          <div key={idx} className="border border-border rounded-2xl p-6 flex flex-col gap-4 bg-background/60">
              ...
              <div>
                ...
                <div className="flex">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={16} strokeWidth={1} className="text-highlight fill-highlight" />
                  ))}
                </div>
              </div>
            </div>
