      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-8 space-y-12">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {plans.map(({ name, price, features, highlight }) => (
            <div
              key={name}
              className={`relative p-8 border rounded-2xl flex flex-col gap-6 card-hover ${
            ...
            >
              ...
              <ul className="space-y-2 flex-1">
                {features.map((feat) => (
                  <li key={feat} className="flex items-start gap-2 text-gray-300 text-sm">
                    <Check size={16} strokeWidth={1} className="text-highlight icon-hover" />
                    {feat}
                  </li>
                ))}
              </ul>
              <Button 
                ...
                size="md" 
                className={highlight ? 'hero-primary-button' : 'hover-unified'}
              >
